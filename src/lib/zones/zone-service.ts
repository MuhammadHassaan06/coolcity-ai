...
function loadDefaultAnalyticsStore(): CanonicalTractRecord[] {
  ...
  // Attempt to load Member 2 canonical processed dataset if available on disk
  const canonicalPath = path.resolve(process.cwd(), "data/processed/phoenix_tract_risk.json");
  if (fs.existsSync(canonicalPath)) {
    try {
      ...
      // Fall through to sample fixture if parsing fails
    }
  }

  // Fallback to Census GEOID test fixture
